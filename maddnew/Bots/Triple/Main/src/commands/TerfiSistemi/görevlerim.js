...
const { maddxRed, maddxb2, maddxb3, maddxd1, maddxd2, maddxd3, maddxTik } = require("../../../../src/configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["gorevler", "görevlerim", "yapmamgerekenler","gorevlerim"],
    name: "görevler",
    help: "görevler @madd/ID",
    category: "yetkili"
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!conf.staffs.some(rol => member.roles.cache.has(rol))) return message.react(maddxRed)
    ...
    function progressBar(value, maxValue, size) {
      const fill = `${maddxd2}`;
      const fillStart = `${maddxd1}`;
      const fillEnd = `${maddxd3}`;
      const empty = `${maddxb2}`;
      const emptyEnd = `${maddxb3}`;
...
${mtask.filter((x) => x.active).map((x) => `\`#${x.id}\` ${x.message} \n${x.completedCount >= x.count ? `${maddxTik}` + " **Tamamlandı!**" : `${progressBar(x.completedCount, x.count, 8)} \`${x.type === "ses" || x.type === "yayin" || x.type == "camera" ? `${moment.duration(x.completedCount).format("H [saat], m [dk], s [sn]")} / ${moment.duration(x.count).format("H [saat], m [dk], s [sn]")}` : `${x.completedCount} / ${x.count}`}\` \nKalan Süre: \`${moment.duration(x.finishDate - Date.now()).format("H [saat], m [dakika] s [saniye]")}\` \nÖdül: **${x.prizeCount} puan**`}`).join("\n\n")}        
