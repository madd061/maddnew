...
const { maddxSonsuz, maddxStar, maddx1, maddx2, maddx3, maddx4, maddx5, maddx6, maddx7, maddx8, maddx9, maddxTik, maddxRed } = require("../../../../src/configs/emojis.json")
...
client.on('interactionCreate', async interaction => {
    ...
    if(interaction.customId === "basvuru") {
        if (!interaction.user.displayName.includes(conf.tag)) {
            interaction.reply({content: `${maddxRed} öncelikle sunucumuzun tagını alman gerekiyor. Tagımız: \`${conf.tag}\``, ephemeral:true});
            return
    };
        ...
        const modal = new Discord.ModalBuilder()
            ...
            const gv3 = new Discord.TextInputBuilder()
                ...
                .setMaxLength(100)
                .setPlaceholder('Örn: Evet oldum. Vencia\'da Moderatördüm')
                .setRequired(true)
        ...
        let s5 = interaction.fields.getTextInputValue('soru5') || "\`Boş Bırakılmış\`";
            await interaction.reply({content: `${maddxTik} Yetkili başvurun başarıyla alındı`, ephemeral: true});
        ...
        if(client.channels.cache.find(x => x.name == "başvuru-log")) {
            ...
            await channelxd2.send(`> ${maddxTik} **${member} Yetkililik Başvurun Kabul Edilmiştir Edilmiştir.**\n> \n> **Kabul Eden Yetkili: ${interaction.user}**`);
            await interaction.update({content:`> ${maddxTik} **Yetkili Başvurusu Başarıyla Kabul Edildi!**\n> **Kabul Eden Yetkili:** ${interaction.user}`,components: []});
            ...
            await channelxd.send(`> ${maddxRed} **${member} Yetkililik Başvurun Red Edilmiştir.**\n> \n> **Red Eden Yetkili: ${interaction.user}**`);
            await interaction.update({content:`> ${maddxRed} **Yetkili Başvurusu Red Edildi!**\n> **Red Eden Yetkili:** ${interaction.user}`,components: []});
