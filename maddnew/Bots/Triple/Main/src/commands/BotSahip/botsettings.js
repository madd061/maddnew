...
run: async (client, message, args, embed) => {
    ...
    const mesaj = await message.channel.send({ embeds: [embed.setDescription(`${maddxTik} Aşağıda sıralanmakta olan botların, ismini veya profil fotoğrafını değiştirmek istediğinz botu seçin.`)], components: [row, row2] });
            ...
            await message.channel.send({ embeds: [embed.setDescription(`${botclient.user} isimli bot üzerinde hangi işlemi yapmak istersin?`)], components: [newrow] }).then(async (msj) => {
                ...
                col.on("collect", async (button) => {
                    ...
                    if (button.customId === "botupdateavatar") {
                        if (msj) msj.edit({ embeds: [embed.setDescription(`${maddxTik} ${botclient.user} isimli botun yeni profil resmini yükleyin veya bağlantısını girin. İşleminizi \`60 saniye\` içinde tamamlamazsanız otomatik olarak iptal edilecektir. İşlemi iptal etmek için \`iptal\` yazabilirsin.`)], components: [] });
                        ...
                        coll.on("collect", async (msg) => {
                            ...
                            botclient.user.setAvatar(avatar).then(() => {
                                ...
                                if (msj) msj.delete().catch(() => { });
                                message.channel.send({ embeds: [embed.setDescription(`${maddxTik} ${botclient.user} isimli botun profil resmi başarıyla güncellendi.`).setThumbnail(botclient.user.avatarURL())] }).then((s) => message.react(maddxTik) && setTimeout(() => { if (s) s.delete(); }, 20000));
                                ...
                                if (bekle) bekle.delete().catch(() => { });
                                if (msj) msj.delete().catch(() => { });
                                message.channel.send({ embeds: [embed.setDescription(`${maddxRed} Profil resmi güncellenemedi, çünkü biraz beklemem gerekiyor.`)] }).then((s) => message.react(maddxRed) && setTimeout(() => { if (s) s.delete(); }, 10000));
                            });
                        });
                        ...
                        if (msj) msj.edit({ embeds: [embed.setDescription(`${maddxTik} ${botclient.user} isimli botun yeni ismini girin. İşleminizi \`60 saniye\` içinde tamamlamazsanız otomatik olarak iptal edilecektir. İşlemi iptal etmek için \`iptal\` yazabilirsin.`)], components: [] });
                        ...
                        coll.on("collect", async (msg) => {
                            ...
                            botclient.user.setUsername(isim).then(() => {
                                if (bekle) bekle.delete().catch(() => { });
                                if (msj) msj.delete().catch(() => { });
                                message.channel.send({ embeds: [embed.setDescription(`${maddxTik} ${botclient.user} isimli botun ismi başarıyla güncellendi.`).addFields({ name: "İsim", value: `\`${eskinick}\` --> \`${botclient.user.username}\``, inline: false })] }).then((s) => message.react(maddxTik) && setTimeout(() => { if(s) s.delete(); }, 20000));
                                const log = client.channels.cache.find(x => x.name == "bot_log")
                                ...
                                if (bekle) bekle.delete().catch(() => { });
                                if (msj) msj.delete().catch(() => { });
                                message.channel.send({ embeds: [embed.setDescription(`${maddxRed} İsim güncellenemedi, çünkü biraz beklemem gerekiyor.`)] }).then((s) => message.react(maddxRed) && setTimeout(() => { if (s) s.delete(); }, 10000));
                            });
                        });
