...
const { maddxRed, maddxTik, secret1, secret2, secret3, secret4, secret5, secret6, secret7, secret8, secret9, secret10, maddxOk } = require("../../../../src/configs/emojis.json");
    ...
    collector.on('end', i => {
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
            ...
            yarak.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 }).then((x) => {
                ...
                interaction.channel.send({ content: `${maddxTik} Başarılı bir şekilde <@${x.values}> adlı üyenin odanıza erişim izni silindi.` });
                });
