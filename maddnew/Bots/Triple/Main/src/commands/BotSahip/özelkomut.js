...
module.exports = {
  ...
  run: async (client, message, args) => {  
    if (!args[0]) return message.channel.send({ content: `${maddxRed} Bir argüman belirtmelisin. (ekle/kaldır/liste)`}).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
    if (args[0] === "oluştur" || args[0] === "ekle") {
        let komutAd = args[1];
        if (!komutAd) return message.reply({ content: `${maddxRed} Bir komut adı belirlemelisin.` }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        if (!args[2] && !args[3]) return message.channel.send({ content: `${maddxRed} Bir yetkili ve verilecek rolü belirlemelisin. \` Yetkili Rol - Verilecek Rol \`` }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        ...
        let kanalcık = args[4];
        if (!kanalcık) return message.reply({ content: `${maddxRed} Bir kanal ismi belirlemelisin. Örnek: vip_log` }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});

        let talents = await özelPerms.findOne({ guildID: message.guild.id, komutAd: komutAd });
        if (talents) return message.reply({ content: `${maddxRed} Bu isimde bir komut zaten mevcut.`}).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        ...
        message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${maddxTik} \`${komutAd}\` adlı komut başarılı bir şekilde oluşturuldu.\n\nKomut İzni Olan Rol(ler): ${yetkiliRol}\nVerilecek Rol(ler): ${roller}\nKomut Log Kanal: ${kanalcık}`)] });
        ...
        if (!data2) return message.reply({ embeds: [new EmbedBuilder().setDescription(`${maddxRed}  Lütfen bir komut adı girerek tekrar deneyiniz.\n\n(Komutlar: \`${data.map(x => x.komutAd).join(" - ")}\`) `)] });
        ...
        if (!data2) return message.reply({ content: `${maddxRed} Hangi komutu silmek istiyorsun?` }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        await özelPerms.deleteOne({ guildID: message.guild.id, komutAd: args[1] })
        await message.channel.send({ content: `${maddxTik} \`${args[1]}\` isimli komut başarılı bir şekilde silindi.`}).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
    }
  }
};