...
const { maddxSonsuz, maddxStar, maddx1, maddx2, maddx3, maddx4, maddx5, maddx6, maddx7, maddx8, maddx9 } = require("../../../../src/configs/emojis.json")
...
${maddxSonsuz} \`(${total} gerçek, ${bonus} bonus, ${leave} ayrılmış, ${fake} fake)\`
      
${maddxSonsuz} \`Günlük: ${daily}, Haftalık: ${weekly}, Taglı: ${tagged}\`
`, ephemeral: true });
}
...
{
await interaction.reply({ content: `
${maddxSonsuz} Sesli kanallardaki üye sayısı : \`${(interaction.guild.members.cache.filter((x) => x.voice.channel).size)}\`
${maddxSonsuz} Sunucudaki toplam üye sayısı : \`${(interaction.guild.memberCount)}\`
${maddxSonsuz} Sunucunun oluşturulma tarihi: \`${moment(interaction.guild.createdAt).locale("tr").format("LLL")}\`
${maddxSonsuz} Sunucu destek numarası : \`${(interaction.guild.id)}\`
`, ephemeral: true });
}
...
${maddxStar} **Mesaj İstatistiği**
${maddxSonsuz} Toplam: \`${messageData ? messageData.topStat : 0}\`

${maddxSonsuz} Haftalık Mesaj: \`${Number(messageWeekly).toLocaleString()} mesaj\`
${maddxSonsuz} Günlük Mesaj: \`${Number(messageDaily).toLocaleString()} mesaj\`
`, ephemeral: true });
}
...
${maddxStar} **Sesli Sohbet İstatistiği**
${maddxSonsuz} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika] s [saniye]")}\`
