...
const { maddxRed, maddxTik, maddxSaat, maddxOk } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client,message, args, embed) => {
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${maddxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(maddxTik)
    ...
    const penal = await client.penalize(message.guild.id, member.user.id, "CHAT-MUTE", true, message.author.id, reason, true, Date.now() + duration);
    if(msg) msg.delete();
    interaction.followUp({ content:`${maddxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar susturuldunuz.`}).catch(() => {});
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${maddxTik} Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
