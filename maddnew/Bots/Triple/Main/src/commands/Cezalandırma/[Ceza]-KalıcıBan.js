...
const { maddxRed, maddxTik, maddxCeza, maddxRevu, maddxOk } = require("../../../../src/configs/emojis.json")
const ayar = require("../../../../src/configs/ayarName.json");
module.exports = {
  conf: {
    aliases: ["forceban"],
    name: "forceban",
    help: "forceban <madd/ID>",
    category: "sahip",
    owner: true
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    message.reply({ content :`${maddxTik} ${member ? member.toString() : user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
