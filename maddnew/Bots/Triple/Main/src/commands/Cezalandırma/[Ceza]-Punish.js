...
const { maddxRed, maddxTik, maddxSaat, maddxOk, maddxCeza, maddxRevu } = require("../../../../src/configs/emojis.json")
...
msg.edit({ content :`${maddxTik} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

const log = new EmbedBuilder() 
.setDescription(`
${maddxCeza} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
${maddxRevu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
${maddxOk} Ban Sebebi: \`${reason}\`
`)
...
if(button.customId === "üç") {
  ...
  msg.edit({ content :`${maddxTik} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`Reklam\` nedeniyle banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

  const log = embed
  .setDescription(`
${maddxCeza} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
${maddxRevu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
${maddxOk} Ban Sebebi: \`Reklam\`
  `)
  .setFooter({ text:`${moment(Date.now()).format("LLL")}    (Ceza ID: #${penal.id})` })
  message.guild.channels.cache.get(conf.banLogChannel).send({ embeds: [log]});
