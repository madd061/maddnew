...
const { maddxRed, maddxTik, maddxRevu, maddxOk, maddxSaat } = require("../../../../src/configs/emojis.json")
...
if (duration) {
  ...
  if(msg) msg.delete();
  await message.channel.send({ content:`${maddxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
...
const penal = await client.penalize(message.guild.id, member.user.id, "TEMP-JAIL", true, message.author.id, reason, true, Date.now() + duration);
if(msg) msg.delete();
interaction.followUp({ content:`${maddxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar jaillendiniz.`}).catch(() => {});
...
await interaction.deferUpdate();
if(msg) msg.delete();
interaction.followUp({ content: `${maddxTik} Jail Atma işlemi başarıyla iptal edildi.`, ephemeral: true });
}
})
}
  },
