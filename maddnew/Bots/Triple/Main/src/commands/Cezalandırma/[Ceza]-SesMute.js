...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${maddxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **sesli kanallarda** susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(maddxTik)
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${maddxTik} Sesli Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
