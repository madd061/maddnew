...
const { maddxKalp, maddxStaff, maddxTik ,maddxStar , maddxd2, maddxb2, maddxd1, maddxb3, maddxd3, maddxRed, maddxSonsuz } = require("../../../../src/configs/emojis.json");
...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if(!conf.staffs.some(rol => message.member.roles.cache.has(rol))) return message.react(maddxRed);
...
${maddxStaff} ${member.toString()}, **Günlük / Haftalık / Toplam Ses İstatistikleri;**

${maddxSonsuz} \` Toplam Ses İstatistiği :  \` **${voiceDaily}**
${maddxSonsuz} \` Haftalık Ses İstatistiği :  \` **${voiceWeekly}**
${maddxSonsuz} \` Günlük Ses İstatistiği :  \` **${voiceDaily}**

${maddxStaff} ${member.toString()}, **Günlük / Haftalık / Toplam Mesaj İstatistikleri;**

${maddxSonsuz} \` Toplam Mesaj İstatistiği : \` **${messageData ? messageData.topStat : 0}**
${maddxSonsuz} \` Haftalık Mesaj İstatistiği : \` **${Number(messageWeekly).toLocaleString()}**
${maddxSonsuz} \` Günlük Mesaj İstatistiği : \` **${Number(messageDaily).toLocaleString()}**

${maddxStaff} ${member.toString()}, **Genel Yetili/Yetki Yükseltim İstatistiklerin Aşağıda Listeleniyor;**

${maddxSonsuz} \` Yetki İlerleme Durumu :  \` ${progressBar(coinData ? coinData.coin : 0, maxValue.coin, 9)}
${maddxSonsuz} \` Yetki Yükselim Puanı :  \` [**${maxValue.coin}**] 
${maddxSonsuz} \` Bulunan Yetkin :  \` ${message.member.roles.highest}
${maddxSonsuz} \` Yükselinecek Yetkin :  \` ${Array.isArray(maxValue.role) ? maxValue.role.length > 1 ? maxValue.role.slice(0, -1).map(x => `<@&${x}>`).join(', ') + ' ve ' + maxValue.role.map(x => `<@&${x}>`).slice(-1) : maxValue.role.map(x => `<@&${x}>`).join("") : `<@&${maxValue.role}>`}
${maddxSonsuz} \` İlerleme Yüzdesi :  \` ilerleme durumun **${atlamaBilgisi}** yapıyorsun.

${maddxStaff} ${member.toString()}, **Görev İstatistiklerin Aşağıda Listeleniyor;**

${maddxSonsuz} \` Görev Alma Tarihi :  \` Bulunamadı. 
${maddxSonsuz} \` Görev Bitiş Tarih :  \` ${mtask.length ? mtask.filter((x) => x.active).map((x) => `**${moment.duration(x.finishDate - Date.now()).format("H [saat], m [dakika] s [saniye]")}**`).join("\n\n") : "Veri bulunamadı."}      
${maddxSonsuz} \` Görev Puanı :  \` **20**
${maddxSonsuz} \` Görev Türü :  \` ${mtask.length ? mtask.filter((x) => x.active).map((x) => `${x.message}`).join("\n\n") : "Veri bulunamadı."}
${maddxSonsuz} \` Görev İlerleme :  \` ${mtask.length === 0 ? "Görev Seçmelisin !" : mtask.filter((x) => x.active).map((x) => `\n${x.completedCount >= x.count ? `${maddxTik}` + " **Tamamlandı!**" : `${progressBar(x.completedCount, x.count, 8)} \`${x.type === "ses" || x.type === "yayin" || x.type == "camera" ? `${moment.duration(x.completedCount).format("H [saat], m [dk], s [sn]")} / ${moment.duration(x.count).format("H [saat], m [dk], s [sn]")}` : `${x.completedCount} / ${x.count}`}\``}`).join("\n\n")}

${maddxStaff} ${member.toString()}, **Top Ses/Mesaj/Kamera/Yayın Sıralaması İstatistiklerin Aşağıda Listeleniyor;**

${maddxSonsuz} \` Top Ses Sıralaması :  \` **${sıralama2}**
${maddxSonsuz} \` Top Mesaj Sıralaması :  \` **${sıralama}**
${maddxSonsuz} \` Top Yayın Sıralaması :  \` **${sıralama3}**
${maddxSonsuz} \` Top Kamera Sıralaması :  \` **${sıralama4}**
`)
...
.addFields({ name:`${maddxStar} **Puan Durumu:**`, value:`
...
`, inline: false },
{ name:`${maddxStar} **Yetki Durumu:**`, value:`
${coinStatus}
`, inline: false })
...
`) 
.addFields({ name:`${maddxStar} **Ceza Kullanımı**`, value: `\`\`\`fix
...
`, inline: false },
{ name:`${maddxStar} **Ceza Puan Detayları:**`, value: `
${maddxKalp} (\` Ban işlemi \`) yerseniz, \`-100\` puan kaybedersiniz.
${maddxKalp} (\` Underworld \`) işlemi yerseniz, \`-75\` puan kaybedersiniz.
${maddxKalp} (\` Karantina/Jail \`) işlemi yerseniz, \`-50\` puan kaybedersiniz.
${maddxKalp} (\` Ses/Yazı \`) Mute işlemi yerseniz, \`-20\` puan kaybedersiniz.
`, inline: false },
{ name:`${maddxStar} **Puan Durumu:**`, value: `
Puanınız: \`${coinData ? Math.floor(coinData.coin) : 0}\`, Gereken Puan: \`${maxValue.coin}\`
${progressBar(coinData ? coinData.coin : 0, maxValue.coin, 9)} \`${coinData ? coinData.coin : 0} / ${maxValue.coin}\`
`, inline: false },
{ name:`${maddxStar} **Yetki Durumu:**`, value:`
...
.addFields({ name:`${maddxStar} **Sesli Sohbet İstatistiği**`, value:`
${maddxKalp} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika]")}\`
${maddxKalp} Public Odalar: \`${await category(conf.publicParents)}\`
${maddxKalp} Secret Odalar: \`${await category(conf.privateParents)}\`
${maddxKalp} Alone Odalar: \`${await category(conf.aloneParents)}\`
${maddxKalp} Yönetim Yetkili Odaları: \`${await category(conf.funParents)}\`
${maddxKalp} Kayıt Odaları: \`${await category(conf.registerParents)}\`
`, inline: false },
{ name:`${maddxStar} **Mesaj İstatistiği**`, value:`
${maddxKalp} Toplam: \`${messageData ? messageData.topStat : 0}\`
${maddxKalp} Haftalık Mesaj: \`${Number(messageWeekly).toLocaleString()} mesaj\`
${maddxKalp} Günlük Mesaj: \`${Number(messageDaily).toLocaleString()} mesaj\`
`, inline: false });
...
function progressBar(value, maxValue, size) {
  ...
  const progressText = maddxd2.repeat(progress);
  const emptyProgressText = maddxb2.repeat(emptyProgress);
  
  return emptyProgress > 0 ? maddxd1+progressText+emptyProgressText+maddxb3 : maddxd1+progressText+emptyProgressText+maddxd3;
};
