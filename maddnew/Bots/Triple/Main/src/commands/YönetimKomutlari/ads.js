...
const { maddxRed, maddxTik, maddxRevu, maddxOk } = require("../../../../src/configs/emojis.json")
  ...
  run: async (client, message, args, embed) => {
    ...
    .then(() => {
      message.channel.awaitMessages({
        ...
        message.reply({ content :`${maddxTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``});
        ...
        }).catch(err => message.reply({ content :`${maddxRed} 2 dakika içerisinde kanıt ekran görüntüsü atılmadığı için iptal edildi.`}));
      });
