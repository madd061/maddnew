...
const { maddxStar, maddxRed, maddxTik } = require("../../../../src/configs/emojis.json");
...
embed.addFields({ name: `${maddxStar} Son Toplantı Bilgisi`, value: `
> Toplantıya **\`${MeetingData.Joining.length}\`** yetkili katılmış ${maddxTik}
> Toplantıya **\`${MeetingData.Leaving.length}\`** yetkili katılmamış ${maddxRed}
...
**${guild.name}** sunucusunda şuan da toplantı durumu: **${meetingStatus ? "Aktif" : "Devre-dışı"}** ${meetingStatus ? maddxTik : maddxRed}`)
...
${maddxStar} **Bitirilen Toplantı Bilgisi**
> Bu toplantıda katılması beklenen **(\`${KatilimData.Leaving.length + KatilimData.Joining.length} yetkili\`)**
> Bu toplantıda katılım sağlayan: **(\`${KatilimData.Joining.length} yetkili\`)** ${maddxTik}
> Bu toplantıda katılım sağlamayan: **(\`${KatilimData.Leaving.length} yetkili\`)** ${maddxRed}
