...
const { maddxOk, maddxTik, maddxRed ,maddxStar } = require("../../../../src/configs/emojis.json");
...
`})
    ...
    collector.on("collect", async (button) => {
      ...
      .setDescription(`${maddxTik} ${member.toString()} üyesinin isim geçmişi ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`)
...
msg.edit({
  ...
  if(button.customId === "cezapuan_sıfırla") {
    ...
    .setDescription(`${maddxTik}  ${member.toString()} üyesinin ceza puanı ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`) 
...
msg.edit({
 ...
 if(button.customId === "sicil_sıfırla") {   
    ...
    .setDescription(`${maddxTik}  ${member.toString()} üyesinin sicili ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`) 
...
msg.edit({
  ...
  components : []
    ...
    if(button.customId === "stat_sıfırla") {   
      ...
      .setDescription(`${maddxTik}  ${member.toString()} üyesinin stat verileri ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`) 
 ...
 if(button.customId === "iptal_button") {   
    ...
    .setDescription(`${maddxTik} Sıfırlama işlemi iptal edildi`) 
