...
module.exports = async (message) => {
    ...
    if (message.member.roles.cache.some(rol => komut.YetkiliRol.includes(rol.id)) || message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        if (!üye) return message.reply({ embeds: [new EmbedBuilder().setDescription(`${maddxRed} Lütfen rol verilecek kişiyi etiketle.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        if (!komut.verilecekRol.some(rol => üye.roles.cache.has(rol))) {
            üye.roles.add(komut.verilecekRol)
            message.react(maddxTik)
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${maddxTik} Başarılı şekilde ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü verdim.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            ...
            message.react(maddxTik)
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${maddxTik} Başarılı şekilde ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü aldım.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
