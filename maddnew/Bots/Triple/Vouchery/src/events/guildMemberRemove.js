...
const { maddxStar, maddxTik, maddxRed,maddxCikis } = require("../../../src/configs/emojis.json")
...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.send({ content: `${maddxCikis} \`${member.user.tag}\` sunucumuzdan ayrıldı ama kim tarafından davet edildiğini bulamadım. Sunucumuzda **${member.guild.memberCount}** Üye Kaldi `});
  } else {
    ...
    channel.send({ content:`${maddxCikis} \`${member.user.tag}\` sunucumuzdan ayrıldı. ${inviter.tag} tarafından davet edilmişti. (**${total}** daveti) Sunucumuzda **${member.guild.memberCount}** Üye Kaldi `});
