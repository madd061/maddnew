...
const { maddxStar, maddxTik, maddxRed } = require("../../../src/configs/emojis.json")
  ...
  if (oldUser.displayName.includes(conf.tag) && !newUser.displayName.includes(conf.tag)) {
    ...
    if (tagModedata && tagModedata.tagMode === true) {
      const embed = new EmbedBuilder()
        ...
        .setDescription(`
${maddxRed} ${member.toString()} isimli eski taglımız, tagımızı <t:${Math.floor(Date.now() / 1000)}:R> bıraktı.
...
`);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`
${maddxStar} ${member.toString()} isimli üye ailemize katıldı, tagımızı <t:${Math.floor(Date.now() / 1000)}:R> aldı.
